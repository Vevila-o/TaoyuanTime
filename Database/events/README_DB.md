...
更新註記（2026-05-30）：此檔是 app 內副本；最新完整資料庫與 AI tag 狀態請優先看 `Database/README_DB.md`。原本 2 筆 blocking root cause 已修正，最新 dry-run 已完成 30/30，但尚未寫入 `Activity.tags`。
...
- 新版前台 AI tag dry-run 也會使用 `ocr_text`, `ocr_summary`, `fee_description`, `registration_info` 作為輔助證據；OCR 不得覆蓋結構化日期、地點、費用或報名 URL。
...
- AI tag 正式寫入資料庫前必須通過 `scraping/data/output/ai_tag_quality_report.json`；目前未達 34/34，因此不要執行 `--apply`。
